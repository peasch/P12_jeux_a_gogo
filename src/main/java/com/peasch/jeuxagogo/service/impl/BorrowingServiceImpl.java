...
public class BorrowingServiceImpl implements BorrowingService {
    ...
    public BorrowingDto add(UserDto user, GameDto game) {
        BorrowingDto borrowingDto = new BorrowingDto();
        UserDto borrower = userService.findByUsername(user.getUsername());
        borrowingDto.setBorrowerDto(borrower);
        borrowingDto.setCopyDto(copyService.getAvailableCopiesByGameId(game.getId()).get(0));
